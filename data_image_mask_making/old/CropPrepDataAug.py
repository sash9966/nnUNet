...
root_folders = ['Le']
datasetname = 'Dataset023_AvgDiffDataAug'
...
def augment_and_save_rotations(image_data, mask_data, common_name_id):
    """Generates rotated versions of the image and mask at 90, 180, and 270 degrees."""
    rotations = [90, 180, 270]
    
    for angle in rotations:
        # Rotate the image and mask
        rotated_image = np.rot90(image_data, k=angle // 90)
        rotated_mask = np.rot90(mask_data, k=angle // 90)
        
        # Set the output filenames for the rotated versions
        rotated_common_name = f"{common_name_id}_rot{angle}"
        mask_output_filename = os.path.join(output_mask_folder, f"{rotated_common_name}.nii.gz")
        image_output_filename = os.path.join(output_image_folder, f"{rotated_common_name}_0000.nii.gz")
        
        # Save the rotated mask and image
        nib.save(nib.Nifti1Image(rotated_mask, np.eye(4)), mask_output_filename)
        nib.save(nib.Nifti1Image(rotated_image, np.eye(4)), image_output_filename)
        
        print(f'Saved rotated mask {angle}°: {mask_output_filename}')
        print(f'Saved rotated image {angle}°: {image_output_filename}')
        
        # Optionally, save inspection plots for rotated versions
        save_inspection_plots(rotated_image, rotated_mask, rotated_common_name)
                        ...
                        if quality_data['Image Quality'].str.contains('Good Image').sum() == 3:
                            ...
                            for i in range(1, 4):  # mask_001.nii to mask_003.nii and image_001.nii to image_003.nii
                                ...
                                if os.path.exists(mask_file) and os.path.exists(image_file):
                                    ...
                                    augment_and_save_rotations(image_output_filename, mask_output_filename, common_name_id)
