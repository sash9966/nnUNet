...
root_folders = ['Le']
datasetname = 'Dataset007_LVandIPspredicitonallimagesCombind'
                        ...
                        if quality_data['Image Quality'].str.contains('Good Image').sum() == 3:
                            ...
                            for i in range(1, 4):  # mask_001.nii to mask_003.nii and image_001.nii to image_003.nii
                                ...
                                if os.path.exists(mask_file) and os.path.exists(avg_image_file) and os.path.exists(mean_diff_file) and os.path.exists(eigenvector_file):
                                    ...
                                    eigenvector_data = normalize_image(eigenvector_data)
                                    ...
                                    nib.save(nib.Nifti1Image(eigenvector_data[:, :, 0], eigenvector_img.affine), 
                                             os.path.join(output_image_folder, f'{common_name_id}_0002.nii.gz'))
                                    nib.save(nib.Nifti1Image(eigenvector_data[:, :, 1], eigenvector_img.affine), 
                                             os.path.join(output_image_folder, f'{common_name_id}_0003.nii.gz'))
                                    nib.save(nib.Nifti1Image(eigenvector_data[:, :, 2], eigenvector_img.affine), 
                                             os.path.join(output_image_folder, f'{common_name_id}_0004.nii.gz'))
