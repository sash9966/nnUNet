...
root_folders = ['Le']
datasetname = 'Dataset012_LVandIPspredicitonallimagesTensorsRGBtoGreyscaleLVLast'
...
def process_mask_slices(mask_data):
    """
                    ...
                    if os.path.exists(excel_path):
                        ...
                        if quality_data['Image Quality'].str.contains('Good Image').sum() == 3:
                            ...
                            for i in range(1, 4):  # mask_001.nii to mask_003.nii and image_001.nii to image_003.nii
                                ...
                                combined_mask = process_mask_slices(mask_data)
                                ...
                                if os.path.exists(mask_file) and os.path.exists(avg_image_file) and os.path.exists(mean_diff_file) and os.path.exists(eigenvector_file):
                                    ...
                                    grayscale_image = cv2.cvtColor(eigenvector_data.astype(np.float32), cv2.COLOR_RGB2GRAY)
                                    print(f'shape of graysclae_image : {np.shape(grayscale_image)}')
                                    ...
                                    eigenvector_data = normalize_image(grayscale_image)
                                    ...
                                    #ADAPT HERE! Change from three seperate slices, combine the 3 slices and use opencv rgb to greyscale transfer!
                                    # Save Eigenvector Image (use one channel for now) as _0002
                                    nib.save(nib.Nifti1Image(eigenvector_data, eigenvector_img.affine), 
                                    ...
                                    save_inspection_plots(np.stack([avg_image_data, mean_diff_data, eigenvector_data], axis=-1), 
