...
root_folders = ['Hannum']
datasetname = 'Dataset054_DataAugAvgDiffCropnoVol3'
                        ...
                        for i in range(1, 4):
                            ...
                            if not slice_data.empty and slice_data.iloc[0]['Image Quality'] == 'Good Image Quality':
                                ...
                                if os.path.exists(mask_file) and os.path.exists(image_file):
                                    ...
                                    rotated_data = rotate_images_and_mask(image_data_list, mask_data)

                                    # Save each rotated version
                                    for degree, rotated_images, rotated_mask in rotated_data:

                                        common_name_id = f'{root_folder}_{volunteer_folder}_{divo_folder}_{degree}_slice_00{i}'
                                        ...
                                        nib.save(nib.Nifti1Image(rotated_mask, mask_img.affine), mask_output_filename)
                                        nib.save(nib.Nifti1Image(rotated_images[0], image_img.affine), image_output_filename)
                                        ...
                                        save_inspection_plots(rotated_images[0], rotated_mask, common_name_id)
                                else:
